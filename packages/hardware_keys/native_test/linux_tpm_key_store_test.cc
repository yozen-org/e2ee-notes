...
using hardware_keys::TpmContext;
using hardware_keys::TpmKeyStore;
using hardware_keys::TpmSealedBlob;
...
void StartEmulator(ESYS_CONTEXT* context) {
  ...
  if (status != TPM2_RC_INITIALIZE) hardware_keys::CheckTpm(status, "Start emulator");
}
